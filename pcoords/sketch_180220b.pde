...
// tooltips hovering
 ...
 public void drawCoords() {
   ...
   float x = 1000 * margin * 1.5;
   float interval = 1000 * (1 - 2*margin) / (dnames.length -1); // interval is dependent on dimensions


   fore = createGraphics(1000, 700);
 ...
 public void updateBack() {
   float x = 1000 * margin * 1.5;
   float interval = 1000 * (1 - 2*margin) / (dnames.length -1); // interval is dependent on dimensions displayed
   float h0, h1;
...
public void drawBezier(float x, int i, float h0, float h1, float interval, boolean hover) {
 ...
 public void drawAxis(float x, float interval) {
    for(int i = 0; i < dnames.length; i++) {
     int index = order[i];
      ...
      fore.stroke(0);
      fore.line(x, 700 * margin, x, 700 * (1- margin));
      ...
      //label it at the top
      fore.text(dnames[index], x, 700 * margin - 15);
      ...
      if (orient[index]) {
        fore.text(max[index], x, 700 * margin - 3);
        if (i == 0) {
          fore.textAlign(LEFT, TOP);
        } else {
            fore.textAlign(CENTER, TOP);
        }        fore.text(min[index], x, 700 * (1-margin) + 3);
      } else {
        fore.text(min[index], x, 700 * margin - 3);
        if (i == 0) {
          fore.textAlign(LEFT, TOP);
        } else {
            fore.textAlign(CENTER, TOP);
        }
        fore.text(max[index], x, 700 * (1-margin) + 3);
      }
      fore.endDraw();
      ...
      interact.rect(x, 700/2, 10, 700* (1-2*margin)+ 1); //CHANGE THE WIDTH
      interact.endDraw();
      x+= interval;
 ...
 public void flip() {
  ...
  public void moveAxis(int start, float endX) {
     float interval = 1000 * (1 - 2*margin) / (dnames.length -1); // interval is dependent on dimensions displayed
     int end = max(0, (int) round((endX - 1000 * margin) / interval));
     while(start != end) {
 ...
 public void resetCanvas() {
    fore = createGraphics(1000, 700);
    interact = createGraphics(1000, 700);
    back = createGraphics(1000, 700);
    updateBack();
 }
